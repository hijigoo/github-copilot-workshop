...
> 📝 **출력된 메이저 버전 번호(17, 21 등)를 기억해 두세요.**
> 이후 Spring Initializr 프로젝트 생성, `build.gradle.kts` 설정, 트러블슈팅에서 동일한 버전을 사용합니다.
...
3. **Version**: `java -version`에서 확인한 메이저 버전 선택 (17, 21 등)
4. **Vendor**: `Eclipse Temurin` (또는 원하는 벤더) 선택
5. **Download** 클릭

> 📸 **[IntelliJ 스크린샷]** Project Structure > SDK > Download JDK 화면에서 JDK를 선택하여 다운로드하는 모습
>
...
> ⚠️ Spring Initializr에서 선택 가능한 Java 버전은 Spring Boot 버전에 따라 다릅니다.
> `java -version` 결과가 목록에 없으면 가장 가까운 낮은 버전을 선택하세요.
...
#### 생성 후 확인: `build.gradle.kts`의 Java 버전

프로젝트를 생성한 후 `build.gradle.kts` 파일을 열어 Java 버전이 올바르게 설정되었는지 확인하세요:

```kotlin
// java -version에서 17이 출력된 경우
java {
    toolchain {
...
> 💡 `languageVersion`의 숫자가 `java -version`에서 확인한 메이저 버전과 일치하면 됩니다.
...
`{VERSION}`은 `java -version`에서 확인한 메이저 버전(17, 21 등)으로 대체하세요:

```bash
# macOS (Homebrew로 설치한 경우)
./gradlew build -Dorg.gradle.java.home=/opt/homebrew/opt/openjdk@{VERSION}
# 예: ./gradlew build -Dorg.gradle.java.home=/opt/homebrew/opt/openjdk@17
# 예: ./gradlew build -Dorg.gradle.java.home=/opt/homebrew/opt/openjdk@21

# Windows (PowerShell)
.\gradlew build -Dorg.gradle.java.home="C:\Program Files\Eclipse Adoptium\jdk-{VERSION}"
# 예: .\gradlew build -Dorg.gradle.java.home="C:\Program Files\Eclipse Adoptium\jdk-17"
```
...
`Cannot find a Java installation matching: {languageVersion=XX}` 에러가 나는 경우,
IntelliJ 내부에서는 JDK를 찾지만 터미널의 Gradle은 시스템에 설치된 JDK를 찾지 못하는 것입니다.

> 아래 예시의 `{VERSION}`은 `java -version`에서 확인한 메이저 버전(17, 21 등)으로 대체하세요.

#### 해결 방법 A: JAVA_HOME 지정하여 빌드

필요한 JDK가 시스템에 설치되어 있지만 기본 버전이 다른 경우에 사용합니다.
먼저 경로가 존재하는지 확인하세요:

```bash
# macOS — 해당 JDK 경로 존재 확인
ls /opt/homebrew/opt/openjdk@{VERSION}
# 예: ls /opt/homebrew/opt/openjdk@17
```

폴더 내용이 출력되면 해당 JDK가 설치된 것입니다. 다음과 같이 JAVA_HOME을 지정하고 빌드하세요:
...
./gradlew build
# 예: export JAVA_HOME=/opt/homebrew/opt/openjdk@17 && ./gradlew build

# Windows (PowerShell)
$env:JAVA_HOME="C:\Program Files\Eclipse Adoptium\jdk-{VERSION}"; .\gradlew build
```
...
2. 좌측 메뉴에서 **SDKs** 클릭
3. `java -version`에서 확인한 버전의 JDK 선택 → **JDK home path**에 표시된 경로를 복사
