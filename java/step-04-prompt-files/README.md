...
| 폴더 | 설명 |
|------|------|
| `starter/` | Step 3 완성 코드 + SDD 적용 완료 (DTO + priority + pagination) — 여기서 시작하세요 |
...
> 📸 **[IntelliJ 스크린샷]** Chat에서 `/test`를 입력하면 프롬프트 파일이 자동 로드되고, `${input:testTarget}` 변수 입력을 요청하는 화면
>
...
> 📸 **[IntelliJ 스크린샷]** `/spec-implement` 호출 후 `featureDescription` 변수에 기능 설명을 입력하는 화면
>
> ![/spec-implement 프롬프트 호출](./images/step04-spec-implement-invoke.png)
