...
## 출력 형식
1. **설계 요약** — 변경 범위와 영향도
2. **DTO/Entity 파일** — dto/, entity/ 수정/추가
...
- 기능 요구사항을 분석합니다
- dto/ 패키지에 DTO record를 추가합니다
- entity/ 패키지에 Entity를 설계합니다
- 영향받는 파일 목록을 정리합니다
...
- Controller → Service → Repository 순서로 작성
- 기존 코드와의 호환성을 유지합니다
...
요구사항:
- TODO에 즐겨찾기 여부(isFavorite) 필드 추가
- PATCH /todos/{id}/favorite 로 즐겨찾기 토글
- GET /todos?favorite=true 로 즐겨찾기만 필터링
- 즐겨찾기된 TODO가 목록 상단에 표시
```

> 📸 **[IntelliJ 스크린샷]** Chat 하단의 Agent 선택 드롭다운에서 `orchestrator`를 선택하는 화면
>
> ![Orchestrator Agent 선택](./images/step08-orchestrator-select.png)
...
|-------|-----------|-----------|
| 1 | Architect | DTO에 `isFavorite` 필드 + Entity 수정 |
| 2 | Tester | 즐겨찾기 토글, 필터링, 정렬 테스트 |
| 3 | Developer | Controller, Service, Repository 구현 + 테스트 통과 |
| 4 | Reviewer | 코드 품질 리뷰 + 개선 사항 |

> 📸 **[IntelliJ 스크린샷]** Orchestrator Agent가 Phase 1→4 파이프라인을 순서대로 실행하며 각 Phase 완료 시 요약을 출력하는 Chat 패널 화면
>
> ![Multi-Agent 파이프라인 실행](./images/step08-pipeline-execution.png)
