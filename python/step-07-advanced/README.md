# Step 7. 고급 워크플로우 — Plan 기능 + Custom Agent 조합
...
| 폴더 | 설명 |
|------|------|
| `starter/` | Step 6 완성 코드 (Custom Agent 3종 포함: `@reviewer`, `@sdd`, `@refactor`) — 여기서 시작하세요 |
...
3개 기능을 모두 구현하고 전체 테스트가 통과하면, Step 6에서 만든 `reviewer` 에이전트로 품질을 점검합니다.
...
**보너스 트랙**으로 넘어가세요:
→ [Step 8. Multi-Agent 워크플로우](../step-08-multi-agent/README.md)
