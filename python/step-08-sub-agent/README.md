...
## 참고: Sub-Agent 관련 frontmatter 속성

### 🔽 Custom Agent가 Sub-Agent로 호출될 때

다른 Agent가 이 Custom Agent를 Sub-Agent로 호출하는 것을 제어합니다.

| 속성 | 기본값 | 설명 |
|------|--------|------|
| `user-invocable` | `true` | `false` 시 Agent 드롭다운에 숨기고, Sub-Agent로만 호출 가능 |
| `disable-model-invocation` | `false` | `true` 시 다른 Agent의 자동 위임을 차단하고, 사용자 명시적 호출만 허용 |
...
### 🔼 Custom Agent가 Sub-Agent를 호출할 때

이 Custom Agent가 다른 Agent를 Sub-Agent로 호출하기 위한 설정입니다.

| 속성 | 기본값 | 설명 |
|------|--------|------|
| `tools` | 모든 도구 | Sub-Agent를 호출하려면 `'agent'` 포함 필수 |
| `agents` | 모든 Agent | Sub-Agent로 호출 가능한 Agent 목록 제한 |
