...
Copilot Chat에는 Ask, Agent 외에 **Plan 모드**가 있습니다.
큰 작업을 요청할 때 Plan 모드를 사용하면, 바로 코드를 생성하는 대신
**구현 계획(Plan)을 먼저 제시**합니다.

- Chat 입력창 상단의 **모드 선택 드롭다운**에서 `Plan` 모드를 선택
...
모드 선택 드롭다운에서 **Plan** 모드를 선택한 뒤 Chat에 입력:
...
![Plan 결과](../screenshot/step08-plan-result.png)
...
![Start Implementation 버튼](../screenshot/step09-start-implementation.png)
