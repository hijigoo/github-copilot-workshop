...
> ✨ **Agent 모드의 핵심 장점**: Agent 모드에서 이 프롬프트를 실행하면, Copilot이 테스트 파일만 생성하는 것이 아니라 **테스트를 실행하고, 실패를 확인한 후, 테스트를 통과시키기 위한 실제 소스코드(`schemas.py`, `main.py` 등)까지 자동으로 구현**합니다.
> 즉, 태스크 3과 태스크 4가 한 번에 진행될 수 있습니다. 테스트 작성 → 테스트 실행 → 실패 확인 → 소스코드 구현 → 테스트 재실행 → 통과까지 Agent가 자동으로 반복합니다.
>
> 하지만 이 튜토리얼에서는 **SDD 흐름을 단계별로 체험**하기 위해 태스크 3(테스트)과 태스크 4(구현)를 분리해서 진행합니다.
