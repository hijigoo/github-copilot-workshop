...
> 📌 **`.specs/` 폴더명에 대해**
> `.specs/`는 자동으로 감지되는 특수한 폴더가 아닙니다.
> `specs/`, `docs/` 등 자유롭게 변경 가능합니다.
...
Copilot Chat(**Agent 모드**)에서 스펙을 분석합니다:
...
> 💡 Agent 모드에서는 테스트 작성 후 자동으로 소스코드 구현까지 이어질 수 있지만, SDD 흐름 체험을 위해 태스크 3(테스트)과 태스크 4(구현)를 분리합니다.
