...
다음을 포함해줘:
- 클라이언트 → main.py (라우트) → 비즈니스 로직 → 저장소 흐름
- 각 계층의 주요 함수명
- 화살표에 주요 동작 표시
...
```
TODO 생성(POST /todos) API의 처리 흐름을 
Mermaid 시퀀스 다이어그램으로 그려줘.

Client → FastAPI → 비즈니스 로직 → 저장소 순서로,
...
```mermaid
graph LR
    Client[클라이언트] --> Router[FastAPI Router]
    Router --> Logic[비즈니스 로직]
    Logic --> Store[저장소]
```
...
> 💡 **참고**: GitHub, VS Code 모두 Mermaid 렌더링을 지원합니다.
...
→ [Step 12 — Docker](../step-12-bonus-a-docker/README.md)
