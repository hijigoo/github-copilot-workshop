...
기반으로 작성된 테스트입니다. SQLite 데이터베이스를 사용하는 버전입니다.
...
def test_생성_유효한데이터_201반환(client):
...
def test_생성_기본우선순위_medium설정(client):
...
def test_생성_마감일포함_마감일저장됨(client):
...
def test_생성_마감일없음_None반환(client):
...
def test_조회_페이지네이션_첫페이지(client):
...
def test_조회_페이지네이션_마지막페이지(client):
...
def test_필터_우선순위_해당항목만반환(client):
...
def test_필터_우선순위와페이지네이션_조합(client):
...
def test_수정_존재하는항목_200반환(client):
    """존재하는 TODO를 PUT으로 수정하면 200과 수정된 항목을 반환합니다."""
    # Given: TODO가 1개 생성된 상태
    create_response = client.post("/todos", json={"title": "원래 제목"})
...
def test_수정_우선순위변경_반영됨(client):
    """PUT으로 우선순위를 변경하면 변경된 값이 반영됩니다."""
    # Given: medium 우선순위의 TODO가 1개 생성된 상태
    create_response = client.post(
        "/todos", json={"title": "할 일", "priority": "medium"}
    )
    todo_id = create_response.json()["id"]

    # When: PUT으로 우선순위를 high로 변경 요청
    response = client.put(f"/todos/{todo_id}", json={"priority": "high"})
...
def test_수정_마감일변경_반영됨(client):
    """PUT으로 마감일을 변경하면 변경된 값이 반영됩니다."""
    # Given: 마감일이 없는 TODO가 1개 생성된 상태
    create_response = client.post("/todos", json={"title": "할 일"})
    todo_id = create_response.json()["id"]

    # When: PUT으로 마감일 변경 요청
    due = "2026-06-30T18:00:00"
    response = client.put(f"/todos/{todo_id}", json={"due_date": due})
...
def test_부분수정_완료상태변경_200반환(client):
...
def test_부분수정_제목변경_제목만수정됨(client):
    """PATCH로 제목만 변경하면 제목만 수정되고 나머지 필드는 유지됩니다."""
    # Given: TODO가 1개 생성된 상태
    create_response = client.post(
        "/todos", json={"title": "원래 제목", "description": "원래 설명"}
    )
    todo_id = create_response.json()["id"]

    # When: PATCH로 제목만 변경 요청
    response = client.patch(f"/todos/{todo_id}", json={"title": "새 제목"})
...
def test_부분수정_마감일변경_마감일만수정됨(client):
    """PATCH로 마감일만 변경하면 마감일만 수정되고 나머지 필드는 유지됩니다."""
    # Given: TODO가 1개 생성된 상태
    create_response = client.post(
        "/todos", json={"title": "할 일", "priority": "high"}
    )
    todo_id = create_response.json()["id"]

    # When: PATCH로 마감일만 변경 요청
    due = "2026-09-15T10:00:00"
    response = client.patch(f"/todos/{todo_id}", json={"due_date": due})
...
def test_부분수정_없는항목_404반환(client):
...
def test_삭제_존재하는항목_204반환(client):
    """존재하는 TODO를 삭제하면 204를 반환하고 목록에서 제거됩니다."""
    # Given: TODO가 1개 생성된 상태
    create_response = client.post("/todos", json={"title": "삭제할 항목"})
    todo_id = create_response.json()["id"]
